            ...
            <div className="grid md:grid-cols-2 gap-4" data-testid="parent-packs">
              {packs.map((p) => (
                <div key={p.id} className="rounded-2xl border border-white/10 bg-[#0a0514]/60 p-6">
                  <div className={`overline ${p.courses_unlocked === p.courses_total && p.courses_total > 0 ? "text-[#00ff66]" : "text-white/40"}`}>{p.courses_total ? `${p.courses_unlocked}/${p.courses_total} unlocked` : "Notes only"}</div>
