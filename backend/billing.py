"""Tutor Pack unlocks — what a learner has paid for, and what stays locked.

Replaces the old pack tiers (basic / premium / xpoints). Access is now per pack and per
learner rather than a property of the pack itself:

  * NOTES are free on every chapter of every pack -- the sample that shows the pack is
    worth buying.
  * Every other content type (mind map, summary, flashcards, quiz) and the Socratic tutor
    are locked until the learner holds an entitlement for that pack.

Pricing: MYR 15 unlocks one pack, every chapter and every content type. Additional packs
bought in the same order are MYR 5 each.
...
looks like a working feature. Two things close the loop today:
  ...
  * an admin can grant a pack to a learner (`POST /billing/grant`), which is what makes the
...
CURRENCY = "MYR"
PRICE_FIRST_PACK = 15
PRICE_ADDITIONAL_PACK = 5

# The one content type that is readable without paying, on every chapter of every pack.
...
async def ensure_indexes(target_db):
    await target_db.entitlements.create_index([("user_id", 1), ("pack_id", 1)], unique=True)
    await target_db.billing_orders.create_index([("user_id", 1), ("created_at", -1)])


async def entitled_pack_ids(user_id: str) -> set:
    """Every pack this learner has unlocked. One query, so callers can check many packs
    without a round trip each."""
    docs = await db.entitlements.find({"user_id": user_id}, {"_id": 0, "pack_id": 1}).to_list(500)
    return {d["pack_id"] for d in docs}


async def has_entitlement(user_id: str, pack_id: Optional[str]) -> bool:
    if not pack_id:
        return False
    return await db.entitlements.find_one({"user_id": user_id, "pack_id": pack_id}) is not None


async def can_access(user: dict, pack_id: Optional[str], content_type: str) -> bool:
    ...
    if is_free_type(content_type):
        return True
    return await has_entitlement(user["id"], pack_id)


def price_for(pack_count: int) -> int:
    """First pack MYR 15, each additional pack in the same order MYR 5."""
    if pack_count <= 0:
        return 0
    return PRICE_FIRST_PACK + (pack_count - 1) * PRICE_ADDITIONAL_PACK


async def _fulfil_order(order_id: str) -> int:
    """Grants every pack on an order and marks it paid. THIS is the function a payment
    ...
    granted = 0
    for pack_id in order.get("pack_ids", []):
        res = await db.entitlements.update_one(
            {"user_id": order["user_id"], "pack_id": pack_id},
            {"$setOnInsert": {
                "id": str(uuid.uuid4()),
...
class CheckoutIn(BaseModel):
    # The pack being unlocked, plus any extra packs bundled into the same order at the
    # add-on price.
...
class GrantIn(BaseModel):
    user_id: str
    pack_ids: List[str] = Field(min_length=1, max_length=50)
...
async def get_pricing(_: dict = Depends(get_current_user)):
    return PricingOut(
        currency=CURRENCY,
        first_pack=PRICE_FIRST_PACK,
        additional_pack=PRICE_ADDITIONAL_PACK,
...
async def my_entitlements(user: dict = Depends(get_current_user)):
    return EntitlementsOut(
        pack_ids=sorted(await entitled_pack_ids(user["id"])),
        free_content_types=sorted(FREE_CONTENT_TYPES),
    )
...
async def checkout(payload: CheckoutIn, user: dict = Depends(get_current_user)):
    ...
    unique = list(dict.fromkeys(payload.pack_ids))  # de-dupe, keep order
    found = await db.packs.count_documents({"id": {"$in": unique}})
    if found != len(unique):
        raise HTTPException(status_code=404, detail="One or more Tutor Packs no longer exist")

    already = await entitled_pack_ids(user["id"])
    to_buy = [p for p in unique if p not in already]
    if not to_buy:
        raise HTTPException(status_code=400, detail="You already have access to these Tutor Packs")
...
async def grant(payload: GrantIn, _: dict = Depends(require_role("admin"))):
    """Admin-side unlock. Covers manual/offline payment and makes the locked-content flow
    testable end to end before a gateway exists."""
    now = datetime.now(timezone.utc).isoformat()
    granted = 0
    for pack_id in dict.fromkeys(payload.pack_ids):
        res = await db.entitlements.update_one(
            {"user_id": payload.user_id, "pack_id": pack_id},
            {"$setOnInsert": {
                "id": str(uuid.uuid4()),
